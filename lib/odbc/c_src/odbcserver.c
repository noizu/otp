...
/*
  ...
  Command protocol between Erlang and C
   ...
   OffSet - integer
   N - integer
   Binary - binary encoded tuple of {SQLQuery, NoRows, Parameters}
   NoRows - integer
...
//---------------------------------------------------------
// Forward Declarations: Main Functions
//---------------------------------------------------------
...
__db_handle__ database_handler(const char *port);

//---------------------------------------------------------
// Forward Declarations: ODBC Functions
//---------------------------------------------------------
...
static db_result_msg encode_result_set(SQLSMALLINT num_of_columns, db_state *state);

static db_result_msg encode_out_params(db_state *state, int cols, param_array *params, int num_param_values);

static db_result_msg encode_column_name_list(SQLSMALLINT num_of_columns, db_state *state);

static db_result_msg encode_value_list(SQLSMALLINT num_of_columns, db_state *state);

static db_result_msg
encode_value_list_scroll(SQLSMALLINT num_of_columns, SQLSMALLINT Orientation, SQLINTEGER OffSet, int N,
                         db_state *state);

static db_result_msg encode_row_count(SQLINTEGER num_of_rows, db_state *state);

static void encode_column_dyn(db_column column, int column_nr, db_state *state);

static void encode_data_type(SQLSMALLINT sql_type, SQLINTEGER size, SQLSMALLINT decimal_digits, db_state *state);

static Boolean
decode_params(db_state *state, byte *buffer, int *index, param_array **params, int i, int j, int num_param_values);
...
static ERL_SOCKET connect_to_erlang(const char* port);
static void send_msg(db_result_msg* msg, ERL_SOCKET socket);
static byte* receive_msg(ERL_SOCKET socket);
static Boolean receive_msg_part(ERL_SOCKET socket, byte* buffer, size_t msg_len);
static Boolean send_msg_part(ERL_SOCKET socket, byte* buffer, size_t msg_len);
static void close_socket(ERL_SOCKET socket);
#endif
...
//---------------------------------------------------------
static void init_driver(int erl_auto_commit_mode, int erl_trace_driver, db_state *state);

static void init_param_column(param_array *params, byte *buffer, int *index, int num_param_values, db_state *state);

static void init_param_statement(int cols, SQLLEN num_param_values, db_state *state, param_status *status);
...
static param_array *bind_parameter_arrays(byte *buffer, int *index, int cols, int num_param_values, db_state *state);

static void *retrive_param_values(param_array *Param);

static db_column retrive_binary_data(db_column column, int column_nr, db_state *state);
...
/*!
 * @brief main entry point of odbc socket program.
 */
int main(void) {
    byte *msg = NULL;
    ...
    temp = strtok(msg, ";");
    if (temp == NULL) DO_EXIT(EXIT_STDIN_BODY);
    length = strlen(temp);
...
#elif defined(UNIX)
    pthread_t thread;
    int result = pthread_create(&thread, NULL, (void* (*)(void*))supervise, (void*)port);
    if (result != 0) DO_EXIT(EXIT_THREAD);
#endif
}

/*!
 * @brief verify port reachable.
 */
void supervise(const char *port) {
    ...
    reason = (msg[0] == SHUTDOWN) ? EXIT_SUCCESS : EXIT_FAILURE; // EXIT_FAILURE Should Not Happen.
    free(msg);
    close_socket(socket);
    clean_socket_lib();
    DO_EXIT(reason);
...
/*!
 * @brief connects to port and handle incoming erl requests.
 */
__db_handle__ database_handler(const char *port) {
    db_result_msg msg;
    ...
    db_state state = {
            ...
            .columns = NULL,
            .number_of_columns = 0,
            .dynamic_buffer = {.buff = NULL, .buffsz = 0, .index = 0},
            .associated_result_set = FALSE,
            .use_srollable_cursors = FALSE,
            .tuple_row = FALSE,
            .binary_strings = FALSE,
            .exists_more_result_sets = FALSE,
            .param_query = FALSE,
            .out_params = FALSE,
    ...
    close_socket(socket);
    clean_socket_lib();
    // Exit will be done by supervisor thread
#ifdef WIN32
    return (DWORD)0;
#endif
}
...
/*!
 * Calls the appropriate function to handle the database
...
static db_result_msg handle_db_request(byte *reqstring, db_state *state) {
    ...
    // First byte is an index that identifies the requested command the rest is the argument string.
...
//---------------------------------------------------------
// ODBC Functions: Implementation
//---------------------------------------------------------

/*!
 * Tries to open a connection to the database using
...
static db_result_msg db_connect(byte *args, db_state *state) {
    ...
    tuple_row(state) = (tuple_row_state == ON) ? TRUE : FALSE;
    binary_strings(state) = (binary_strings == ON) ? TRUE : FALSE;
    use_srollable_cursors(state) = (use_srollable_cursors == ON) ? TRUE : FALSE;
    extended_errors(state) = (extended_errors == ON) ? TRUE : FALSE;

    init_driver(erl_auto_commit_mode, erl_trace_driver, state);

    connlen = (SQLSMALLINT) strlen((const char *) connStrIn);
    result = SQLDriverConnect(connection_handle(state), NULL, (SQLCHAR *) connStrIn, connlen, connStrOut,
                              (SQLSMALLINT) MAX_CONN_STR_OUT, &stringlength2ptr, SQL_DRIVER_NOPROMPT);
    ...
    if (!sql_success(result)) {
        ...
        if (!sql_success(SQLFreeHandle(SQL_HANDLE_DBC, connection_handle(state)))) DO_EXIT(EXIT_FREE);
        if (!sql_success(SQLFreeHandle(SQL_HANDLE_ENV, environment_handle(state)))) DO_EXIT(EXIT_FREE);
        return msg;
    }
     ...
     * BEGIN
     */
    auto_commit_mode = (erl_auto_commit_mode == ON) ? SQL_AUTOCOMMIT_ON : SQL_AUTOCOMMIT_OFF;
    if (!sql_success(
            SQLSetConnectAttr(connection_handle(state), SQL_ATTR_AUTOCOMMIT, (SQLPOINTER) auto_commit_mode, 0))) {
        diagnos = get_diagnos(SQL_HANDLE_DBC, connection_handle(state), extended_errors(state));
...
static db_result_msg db_end_tran(byte compleationtype, db_state *state) {
    ...
    result = SQLEndTran(SQL_HANDLE_DBC, connection_handle(state), (SQLSMALLINT) compleationtype);
...
/*!
 * Executes an sql query and encodes the result set as an
...
static db_result_msg db_query(byte *sql, db_state *state) {
    ...
    if (!sql_success(SQLAllocHandle(SQL_HANDLE_STMT, connection_handle(state), &statement_handle(state))))
        DO_EXIT(EXIT_ALLOC);
    ...
    if (result != SQL_SUCCESS && result != SQL_NO_DATA_FOUND) {
        ...
        if (strcmp((char *) diagnos.sqlState, INFO) == 0) {
            ...
            if ((strcmp((char *) is_error, "error") == 0)) {
                msg = encode_error_message((char *) diagnos.error_msg, extended_error(state, diagnos.sqlState),
                                           diagnos.nativeError);
                clean_state(state);
                return msg;
            }
...
/*!
 * Executes an sql query. Returns number of rows in the result set.
 */
static db_result_msg db_select_count(byte *sql, db_state *state) {
    ...
    if (use_srollable_cursors(state)) {
        ...
        SQLSetStmtAttr(statement_handle(state), (SQLINTEGER) SQL_ATTR_CURSOR_SCROLLABLE, (SQLPOINTER) SQL_SCROLLABLE,
                       (SQLINTEGER) 0);
    }
    ...
    if (!sql_success(SQLNumResultCols(statement_handle(state), &num_of_columns))) DO_EXIT(EXIT_COLS);

    nr_of_columns(state) = (int) num_of_columns;
    columns(state) = alloc_column_buffer(nr_of_columns(state));

    if (!sql_success(SQLRowCount(statement_handle(state), &num_of_rows))) DO_EXIT(EXIT_ROWS);

    return encode_row_count(num_of_rows, state);
}

/*!
 * Fetches rows from the result set associated with the
...
/*!
 * Handles parameterized queries ex:
...
static db_result_msg db_param_query(byte *buffer, db_state *state) {
    ...
    if (params != NULL) {
            ...
            for (i = 0; i < param_status.params_processed; i++) {
                switch (param_status.param_status_array[i]) {
                    ...
                    default:
                        ...
                        msg = encode_error_message(diagnos.error_msg, extended_error(state, diagnos.sqlState),
                                                   diagnos.nativeError);
        ...
        if (!sql_success(SQLFreeStmt(statement_handle(state), SQL_RESET_PARAMS))) {
            DO_EXIT(EXIT_FREE);
        }
    } else {
...
static db_result_msg db_describe_table(byte *sql, db_state *state) {
    ...
    for (i = 0; i < num_of_columns; ++i) {
        if (!sql_success(SQLDescribeCol(statement_handle(state), (SQLUSMALLINT)(i + 1),
                                        name, sizeof(name), &name_len, &sql_type, &size, &dec_digits, &nullable)))
            DO_EXIT(EXIT_DESC);
...
//---------------------------------------------------------
// Encode/Decode Functions: Implementation
//---------------------------------------------------------
...
/*!
 * Encode an error-message to send back to erlang
 */
static db_result_msg encode_error_message(char *reason, char *errCode, SQLINTEGER nativeError) {
    int index;
    db_result_msg msg;
...
/*!
 * Encode a messge that is a erlang atom
 */
static db_result_msg encode_atom_message(char *atom) {
    int index;
    db_result_msg msg;
...
static db_result_msg encode_result(db_state *state) {
    ...
    if (param_query(state) && update) {
        if (!sql_success(SQLGetInfo(connection_handle(state), SQL_PARAM_ARRAY_ROW_COUNTS, (SQLPOINTER) & paramBatch,
                                    sizeof(paramBatch), NULL))) {
            DO_EXIT(EXIT_DRIVER_INFO);
        }

        if (paramBatch == SQL_PARC_BATCH) {
            // Individual row counts (one for each parameter set) are available, sum them up
            do {
 ...
 *
 */
static db_result_msg encode_out_params(db_state *state, int num_of_params, param_array *params, int num_param_values) {
...
/*!
 * Encodes the result set into the "ei_x" - dynamic_buffer held by the state variable
 */
static db_result_msg encode_result_set(SQLSMALLINT num_of_columns, db_state *state) {
    db_result_msg msg;
...
/*!
 * Encodes the list of column names into the "ei_x" - dynamic_buffer held by the state variable
 */
static db_result_msg encode_column_name_list(SQLSMALLINT num_of_columns, db_state *state) {
    int i;
    db_result_msg msg;
    ...
    for (i = 0; i < num_of_columns; ++i) {
        ...
        if (!sql_success(SQLDescribeCol(statement_handle(state), (SQLSMALLINT)(i + 1), name, sizeof(name), &name_len,
                                        &sql_type, &size, &dec_digits, &nullable)))
            DO_EXIT(EXIT_DESC);
        ...
        if (msg.length > 0) {
            ...
            if (columns(state)[i].type.len > 0) {
                columns(state)[i].buffer = (char *) safe_malloc(columns(state)[i].type.len);
...
/*!
 * Encodes the list(s) of row values fetched by SQLFetch into the "ei_x" - dynamic_buffer held by the state variable
 */
static db_result_msg encode_value_list(SQLSMALLINT num_of_columns, db_state *state) {
    int i;
    SQLRETURN result;
...
/*!
 * Encodes the list(s) of row values fetched with SQLFetchScroll into the "ei_x"
...
/*!
 * Encodes the a column value into the "ei_x" - dynamic_buffer
    ...
    if (column.type.len == 0 ||
        ...
        switch (column.type.c) {
            ...
            case SQL_C_SLONG:
                ...
                break;
            case SQL_C_DOUBLE:
                ei_x_encode_double(&dynamic_buffer(state), *(double *) column.buffer);
                break;
            case SQL_C_BIT:
                ei_x_encode_atom(&dynamic_buffer(state), column.buffer[0] ? "true" : "false");
                break;
            case SQL_C_BINARY:
...
//---------------------------------------------------------
// Erlang Port Communication Functions: Implementation
//---------------------------------------------------------
...
static int read_exact(byte *buffer, int len) {
    ...
    int i, got = 0;
    do {
        if ((i = read(0, buffer + got, len - got)) <= 0) return(i);
        got += i;
    } while (got < len);
    return len;
#else
...
static byte *receive_erlang_port_msg(void) {
    ...
    if (read_exact(lengthstr, LENGTH_INDICATOR_SIZE) != LENGTH_INDICATOR_SIZE) {
...
//---------------------------------------------------------
// Socket Communication Functions: Implementation
//---------------------------------------------------------
...
static Boolean receive_msg_part(ERL_SOCKET socket, byte *buffer, size_t msg_len) {
    ...
    if (nr_bytes_received == msg_len) {
        ...
        return receive_msg_part(socket, buffer + nr_bytes_received, msg_len - nr_bytes_received);
    } else if (nr_bytes_received == -1) {
        return FALSE;
    } else {
        // nr_bytes_received > msg_len
        close_socket(socket);
...
static Boolean send_msg_part(ERL_SOCKET socket, byte *buffer, size_t msg_len) {
    ...
    if (nr_bytes_sent == msg_len) {
        ...
        return send_msg_part(socket, buffer + nr_bytes_sent, msg_len - nr_bytes_sent);
    } else if (nr_bytes_sent == -1) {
        return FALSE;
    } else {
        // nr_bytes_sent > msg_len
        close_socket(socket);
...
//---------------------------------------------------------
// Memory Handling Functions: Implementations
//---------------------------------------------------------
...
static void *safe_malloc(int size) {
    ...
    if (memory == NULL) DO_EXIT(EXIT_ALLOC);
    return memory;
}
...
static db_column *alloc_column_buffer(int n) {
    ...
    for (i = 0; i < n; i++) columns[i].buffer = NULL;

    return columns;
}

/*!
 * Deallocate memory allocated by alloc_column_buffer
 */
static void free_column_buffer(db_column **columns, int n) {
    int i;
    if (*columns != NULL) {
...
/*!
 * Frees resources associated with the current statement handle
 * kept in the state.
 */
static void clean_state(db_state *state) {
...
//---------------------------------------------------------
// Init/Map/Bind/Retrieve Functions: Implementation
//---------------------------------------------------------
...
static void init_driver(int erl_auto_commit_mode, int erl_trace_driver, db_state *state) {
    ...
    auto_commit_mode = (erl_auto_commit_mode == ON) ? SQL_AUTOCOMMIT_ON : SQL_AUTOCOMMIT_OFF;
    trace_driver = (erl_trace_driver == ON) ? SQL_OPT_TRACE_ON : SQL_OPT_TRACE_OFF;
    if (!sql_success(SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &environment_handle(state)))) DO_EXIT(EXIT_ALLOC);
    if (!sql_success(
            SQLSetEnvAttr(environment_handle(state), SQL_ATTR_ODBC_VERSION, (SQLPOINTER) SQL_OV_ODBC3, 0)))
        DO_EXIT(EXIT_ENV);
    if (!sql_success(SQLAllocHandle(SQL_HANDLE_DBC, environment_handle(state), &connection_handle(state))))
        DO_EXIT(EXIT_ALLOC);
    // By default Erlang handles all timeouts
    if (!sql_success(
            SQLSetConnectAttr(connection_handle(state), SQL_ATTR_CONNECTION_TIMEOUT, (SQLPOINTER) TIME_OUT, 0)))
        DO_EXIT(EXIT_CONNECTION);
    if (!sql_success(
            SQLSetConnectAttr(connection_handle(state), SQL_ATTR_AUTOCOMMIT, (SQLPOINTER) auto_commit_mode, 0)))
        DO_EXIT(EXIT_CONNECTION);
    if (!sql_success(SQLSetConnectAttr(connection_handle(state), SQL_ATTR_TRACE, (SQLPOINTER) trace_driver, 0)))
        DO_EXIT(EXIT_CONNECTION);
}
...
static void init_param_column(param_array *params, byte *buffer, int *index, int num_param_values, db_state *state) {
    ...
    switch (user_type) {
        case USER_SMALL_INT:
            ...
            params->values.integer = (SQLINTEGER *) safe_malloc(num_param_values * params->type.len);
            break;
        case USER_INT:
        ...
        case USER_DECIMAL:
        case USER_NMERIC:
            params->type.sql = (user_type == USER_NMERIC) ? SQL_NUMERIC : SQL_DECIMAL;
            ...
            if (params->type.c == SQL_C_SLONG) {
                ...
                params->values.floating = (double *) safe_malloc(num_param_values * params->type.len);
            } else if (params->type.c == SQL_C_CHAR) {
                params->type.strlen_or_indptr_array = alloc_strlen_indptr(num_param_values, SQL_NTS);
                params->values.string = (byte *) safe_malloc(num_param_values * sizeof(byte) * params->type.len);
            }
            break;
        case USER_CHAR:
        case USER_VARCHAR:
            params->type.sql = (user_type == USER_CHAR) ? SQL_CHAR : SQL_VARCHAR;
        ...
        case USER_TIMESTAMP:
            ...
            params->values.string = (byte *) safe_malloc(num_param_values * params->type.len);
            break;
        ...
        case USER_BOOLEAN:
            ...
            params->values.bool = (byte *) safe_malloc(num_param_values * params->type.len);
            break;
    }
    params->offset = 0;
...
static void init_param_statement(int cols, SQLLEN num_param_values, db_state *state, param_status *status) {
    ...
    status->param_status_array = (SQLUSMALLINT *) safe_malloc(num_param_values * sizeof(SQLUSMALLINT));
    ...
    if (!sql_success(SQLSetStmtAttr(statement_handle(state), SQL_ATTR_PARAMSET_SIZE, (SQLLEN *) num_param_values, 0))) {
        DO_EXIT(EXIT_PARAM_ARRAY);
    }

    if (!sql_success(
            SQLSetStmtAttr(statement_handle(state), SQL_ATTR_PARAM_STATUS_PTR, (status->param_status_array), 0))) {
        DO_EXIT(EXIT_PARAM_ARRAY);
    }

    if (!sql_success(
            SQLSetStmtAttr(statement_handle(state), SQL_ATTR_PARAMS_PROCESSED_PTR, &(status->params_processed), 0))) {
        DO_EXIT(EXIT_PARAM_ARRAY);
    }
}
...
static void map_dec_num_2_c_column(col_type *type, int precision, int scale) {
    ...
    if (precision >= 0 && precision <= 4 && scale == 0) {
        ...
        type->len = sizeof(SQLINTEGER);
        type->c = SQL_C_SLONG;
    } else if ((precision >= 10 && precision <= 15 && scale == 0) || (precision <= 15 && scale > 0)) {
...
/*!
 * Transform SQL columntype to C columntype. Returns a dummy
...
static db_result_msg map_sql_2_c_column(db_column *column, db_state *state) {
    ...
    switch (column->type.sql) {
        ...
        case SQL_LONGVARBINARY:
            ...
            column->type.len = (column->type.col_size) + sizeof(byte);
        ...
        case SQL_NUMERIC:
        case SQL_DECIMAL:
            map_dec_num_2_c_column(&(column->type), column->type.col_size, column->type.decimal_digits);
...
/*!
 * More than one call to SQLGetData may be required to retrieve
...
static db_column retrive_binary_data(db_column column, int column_nr, db_state *state) {
    ...
    result = SQLGetData(statement_handle(state), (SQLSMALLINT)(column_nr + 1), SQL_C_CHAR, outputptr, blocklen,
    ...
    while (result == SQL_SUCCESS_WITH_INFO) {
        ...
        if (strcmp((char *) diagnos.sqlState, TRUNCATED) == 0) {
            ...
            column.buffer = safe_realloc((void *) column.buffer, column.type.len);
            outputptr = column.buffer + outputlen;
            result = SQLGetData(statement_handle(state), (SQLSMALLINT)(column_nr + 1), SQL_C_CHAR,
                                outputptr, blocklen, &column.type.strlen_or_indptr);
        }
    }
...
/*!
 * Returns information about support for scrollable cursors
 */
static db_result_msg retrive_scrollable_cursor_support_info(db_state *state) {
    db_result_msg msg;
    ...
    if (use_srollable_cursors(state)) {
        ...
        if (!sql_success(SQLGetInfo(connection_handle(state), SQL_DYNAMIC_CURSOR_ATTRIBUTES1,
                                    (SQLPOINTER) & supportMask, sizeof(supportMask), NULL))) {
            DO_EXIT(EXIT_DRIVER_INFO);
        }
        ...
        // Scrollable cursors disabled by the user
        ei_x_encode_atom(&dynamic_buffer(state), "false");
        ei_x_encode_atom(&dynamic_buffer(state), "false");
    }
    msg.buffer = dynamic_buffer(state).buff;
...
static db_result_msg more_result_sets(db_state *state) {
    ...
    if (sql_success(result)) {
        ...
        // As we found an error we do not care about any potential more result sets.
        exists_more_result_sets(state) = FALSE;
        diagnos = get_diagnos(SQL_HANDLE_STMT, statement_handle(state), extended_errors(state));
        strcat((char *) diagnos.error_msg, "Failed to create on of the result sets");
        msg = encode_error_message(diagnos.error_msg, extended_error(state, diagnos.sqlState), diagnos.nativeError);
        return msg;
    }
}
...
/*!
 * An ODBC function can post zero or more diagnostic records
...
static diagnos get_diagnos(SQLSMALLINT handleType, SQLHANDLE handle, Boolean extendedErrors) {
    ...
    // Foreach diagnostic record in the current set of diagnostic records the error message is obtained
    for (record_nr = 1;; record_nr++) {
        ...
        if (result == SQL_SUCCESS) {
            // update the sqlstate in the diagnos record, because the SQLGetDiagRec call succeeded
    ...
    if (acc_errmsg_size == 0) {
        strcat((char *) diagnos.error_msg, "No SQL-driver information available.");
    } else if (!extendedErrors) {
        strcat(strcat((char *) diagnos.error_msg, " SQLSTATE IS: "), (char *) diagnos.sqlState);
    }
    return diagnos;
}

/*!
 * lowercase string. (including index[len]).
 */
static void str_tolower(char *str, int len) {
    int i;
    for (i = 0; i <= len; i++) {
